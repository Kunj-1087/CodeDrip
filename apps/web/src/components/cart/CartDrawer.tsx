      ...
      {open && (
        <div className="fixed inset-0 z-[95]" role="dialog" aria-modal="true" aria-label="Your cart">
          ...
          <motion.div 
            ...
            className="absolute right-0 top-0 flex h-full w-full flex-col border-l border-white/5 bg-[#0A0A0C] shadow-2xl sm:max-w-md glass-panel gpu-layer"
          >
            {/* Header */}
                  ...
                  <AnimatePresence initial={false}>
                    {items.map((line) => (
                      <motion.li 
                        ...
                        layout
                        className="flex gap-4 py-5 hover:bg-white/[0.01] px-2 rounded-xl transition-colors overflow-hidden"
                      >
                        <Link
                          href={`/shop/${line.slug}`}
                          onClick={onClose}
